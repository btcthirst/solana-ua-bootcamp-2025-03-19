...
describe("escrow1", () => {
  ...
  // Error: Account `makerTokenAccountA` not provided.
  const closeOfferTx = async (
    ...
    const res = await confirmTransaction(connection, transactionSignature);

    return res;
  };

  // Alice closes the offer, and the vault account is closed.
  test("Offer closed by Alice, vault account is closed", async () => {
    const offeredUsdc = new BN(10_000_000);
    const wantedWif = new BN(100_000_000);
