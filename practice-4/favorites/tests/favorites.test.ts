...
describe("favorites", () => {
  ...
  it("Update our favorites to the blockchain!", async () => {
    ...
    // Update our favorites to the blockchain!
    const updFavoriteNumber = new anchor.BN(17);
    const updFavoriteColor = "blue";

    try {
      tx = await program.methods
        // Call the update_favorites instruction handler
        .updateFavorites(updFavoriteNumber, updFavoriteColor)
        .accounts({
          user: user.publicKey,
          // Note that both `favorites` and `system_program` are added
          // automatically.
    ...
    // Calculate the PDA account address that holds the user's favorites
    const [updFavoritesPda, _updFavoritesBump] = web3.PublicKey.findProgramAddressSync(
      [Buffer.from("favorites"), user.publicKey.toBuffer()],
      program.programId
    );

    // And make sure it matches!
    const updDataFromPda = await program.account.favorites.fetch(updFavoritesPda);
    expect(updDataFromPda.color).toEqual(updFavoriteColor);
    expect(updDataFromPda.number.toNumber()).toEqual(updFavoriteNumber.toNumber());
